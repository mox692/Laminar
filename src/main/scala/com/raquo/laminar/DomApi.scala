...
object DomApi {
  ...
  def insertAfter(
    ...
    try {
      ...
      parent.insertBefore(
        newChild = newChild,
        refChild = referenceChild.nextSibling
      )
      true
    } catch {
      case JavaScriptException(_: dom.DOMException) => false
  ...
  /** Note: This walks up the real DOM element tree, not the Laminar DOM tree.
    * See ChildNode.isDescendantOf if you want to walk up Laminar's tree
    * instead.
    */
  @tailrec final def isDescendantOf(
    ...
    val effectiveParentNode = if (node.parentNode != null) {
      ...
      val maybeShadowHost = node
        .asInstanceOf[js.Dynamic]
        .selectDynamic("host")
        .asInstanceOf[js.UndefOr[dom.Node]]
  ...
  def getHtmlAttributeRaw(
    ...
    val domValue =
      element.ref.getAttributeNS(namespaceURI = null, localName = attr.name)
    if (domValue != null) {
      domValue
    } else {
      js.undefined
  ...
  /** Returns `js.undefined` when the property is missing on the element. If the
    * element type supports this property, it should never be js.undefined.
    */
  ...
  def getHtmlPropertyRaw[V, DomV](
    ...
    element.ref
      .asInstanceOf[js.Dynamic]
      .selectDynamic(prop.name)
      .asInstanceOf[js.UndefOr[DomV]]
  }
  ...
  private[laminar] def setHtmlPropertyRaw[V, DomV](
    ...
    element.ref
      .asInstanceOf[js.Dynamic]
      .updateDynamic(prop.name)(value.asInstanceOf[js.Any])
  }

  /** CSS Style Properties */

  /** Note: this only gets inline style values – those set via the `style`
    * attribute, which includes all style props set by Laminar. It does not
    * account for CSS declarations in `<style>` tags.
    *
    * Returns empty string if the given style property is not defined in this
    * element's inline styles.
    *
    * See
  ...
  @inline private[laminar] def setRefStyle(
    ...
    if (styleValue == null) {
      prefixes.foreach(prefix =>
        ref.style.removeProperty(prefix + styleCssName)
      )
  ...
  def createSvgElement[Ref <: dom.svg.Element](tag: SvgTag[Ref]): Ref = {
    dom.document
      .createElementNS(
        namespaceURI = SvgAttr.svgNamespaceUri,
  ...
  def removeSvgAttribute(
    ...
    element.ref.removeAttributeNS(
      namespaceURI =
        attr.namespaceUri.orNull, // Tested that this works in Chrome & FF
      localName = attr.localName
    )
  }
  ...
  def createCommentNode(text: String): dom.Comment =
  ...
  /** #WARNING: Only use on trusted HTML strings. HTML strings can contain
    * embedded Javascript code, which this function will execute blindly!
    *
    * Note: this expects the html string to contain one HTML element, and will
    * throw otherwise (e.g. if given a text node, an SVG, or multiple elements)
    */
  ...
  /** #WARNING: Only use on trusted HTML strings. HTML strings can contain
    * embedded Javascript code, which this function will execute blindly!
    *
    * Note: this expects the html string to contain one element matching the tag
    * name, and will throw otherwise (e.g. if given a text node, an element with
    * a different tag name, or multiple elements)
    */
  ...
  /** #WARNING: Only use on trusted SVG strings. SVG strings can contain
    * embedded Javascript code, which this function will execute blindly!
    *
    * Note: this expects the svg string to contain one HTML element, and will
    * throw otherwise (e.g. if given a text node, HTML, or multiple elements)
    */
  ...
  /** #WARNING: Only use on trusted SVG strings. SVG strings can contain
    * embedded Javascript code, which this function will execute blindly!
    *
    * Note: this expects the svg string to contain one element matching the tag
    * name, and will throw otherwise (e.g. if given a text node, an element with
    * a different tag name, or multiple elements)
    */
  ...
  /** #WARNING: Only use on trusted HTML strings. HTML strings can contain
    * embedded Javascript code, which this function will execute blindly!
    *
    * Note: This method does not work in Internet Explorer. See
    * https://stackoverflow.com/q/10585029 for the issues with various
    * approaches. Use this if you need IE support:
    * https://gist.github.com/Munawwar/6e6362dbdf77c7865a99
    */
  def unsafeParseHtmlStringIntoNodeArray(
    ...
    val arr = js.Array.from(
      htmlParserContainer.content.childNodes.asInstanceOf[js.Iterable[dom.Node]]
    )
  ...
  def unsafeParseSvgStringIntoNodeArray(
    ...
    // #TODO add to `ew`: js.Array.from(nodeList)
    val arr = js.Array.from(
      svgParserContainer.childNodes.asInstanceOf[js.Iterable[dom.Node]]
    )
  ...
  def assertSingleNode(
    ...
    if (nodes.length != 1) {
      throw new Exception(
        s"$clue: expected exactly 1 element, got ${nodes.length}"
      )
    }
    nodes(0)
  }

  def assertHtmlElement(
      node: dom.Node,
      clue: String = "Error"
  ): dom.html.Element = {
    node match {
      ...
      case _ =>
        throw new Exception(
          s"$clue: expected HTML element, got non-element node: ${node.nodeName}"
        )
    }
  }

  def assertSvgElement(
      node: dom.Node,
      clue: String = "Error"
  ): dom.svg.Element = {
    node match {
      ...
      case _ =>
        throw new Exception(
          s"$clue: expected SVG element, got non-element node: ${node.nodeName}"
        )
    }
  }
    ...
    node match {
      case element: dom.Element =>
        if (tag.name.ew.toLowerCase() != element.tagName.ew.toLowerCase()) {
          throw new Exception(
            s"$clue: expected tag name `${tag.name}`, got `${element.tagName}`"
          )
        } else {
      ...
      case _ =>
        throw new Exception(
          s"$clue: expected element, got non-element node: ${node.nodeName}"
        )
    }
  }
  ...
  /** @return
    *   hierarchical path describing the position and identity of this node,
    *   starting with the root.
    */
  @tailrec def debugPath(
      element: dom.Node,
      initial: List[String] = Nil
  ): List[String] = {
    element match {
      case null => initial
      case _ =>
        debugPath(element.parentNode, debugNodeDescription(element) :: initial)
    }
  }
